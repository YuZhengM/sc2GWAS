...
def get_score(trs_file, name, cell_rate, _type_, value):
    ...
    name_value = name_data.layers[value]

    values = np.array(csr_matrix(name_value).todense()).flatten()
    values_index = np.argsort(values)
    name_data = name_data[values_index, :]
    print(name_data.obs["barcode"].tolist())
    ...
    if _type_ == "umap":
        ...
        print(name_data.obs["TSNE2"].tolist())

    print(list(values[values_index]))


if __name__ == '__main__':
    get_score(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])
